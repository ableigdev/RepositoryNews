...
QByteArray AESEncryption::sha256(const QByteArray& text)
{
...
QByteArray AESEncryption::encrypt(const QByteArray& data, const QByteArray& password)
{
    int outLen = 0;
    QByteArray dataBuff;
...
QByteArray AESEncryption::decrypt(const QByteArray& data, const QByteArray& password)
{
    int outLen = 0;
    QByteArray dataBuff;
    dataBuff.resize(data.size() + AES_BLOCK_SIZE);
